...
---@class Darkrit.Graphics.PhysicsTransformBox2D
...
---@field private _position NVec The current position (internal cached).
---@field rotation number The current rotation.
local physics_transform_box2d = {}
physics_transform_box2d.__index = physics_transform_box2d
...
---@return Darkrit.Graphics.PhysicsTransformBox2D
function physics_transform_box2d.new(body, base_transform)
    local self = setmetatable({}, physics_transform_box2d)
    self.body = body
    self._position = nvec(0, 0)
...
function physics_transform_box2d.__index(self, key)
    if key == "position" then
        -- For code maintainers, it's importnat to keep this here, as other entities might
        -- change the body position without using this transform
        self._position.x = self.body:getX()
        self._position.y = self.body:getY()
    ...
    elseif key == "scale" then
        ...
        return rawget(physics_transform_box2d, key)
    end
end

function physics_transform_box2d.__newindex(self, key, value)
    if key == "position" then
...
function physics_transform_box2d:translate(translation)
...
---@param angle number
function physics_transform_box2d:rotate(angle)
...
function physics_transform_box2d:scale_by(factor)
...
function physics_transform_box2d:update_scale(newScale)
    ...
    if newScale.x <= 0.05 then newScale.x = 0.01 end
    if newScale.y <= 0.05 then newScale.y = 0.01 end
    ...
    for _, fixture in ipairs(fixtures) do
        ...
        newFixture:setUserData(user_data)
        newFixture:setFilterData(categories, mask, group)
    end
    self.scale = newScale
end

return physics_transform_box2d
